...
const config = JSON.parse(
	fs.readFileSync('configurations.json', {
		encoding: 'utf-8'
	})
);
...
const emailTemplateNames = JSON.parse(
	fs.readFileSync('templates/email/emails.json')
);
let emailTemplates = {};
...
function sendEmail(name, json, callback) {
	if (__dev__ || always) {
		emailTemplates[name].html = pug.compileFile(
			'templates/email/' + emailTemplateNames[email].html
		);
		emailTemplates[name].text = pug.compileFile(
			'templates/email/' + emailTemplateNames[email].text
		);
	}
	if (json.pug == undefined) json.pug = {};
	if (json.email == undefined) throw new Error('No Email contents given');
	json.email.html = emailTemplates[name].html(json.pug);
	json.email.text = emailTemplates[name].text(json.pug);
	if (callback == undefined) return transporter.sendMail(json.email);
	else transporter.sendMail(json.email, callback);
}
...
const babelPath = './babeljs/';
const compiledBabelPath = './www/js/';
function renderJS(js) {
	if (__dev__) {
		let code = babel.transformFileSync(babelPath + js).code;
		fs.writeFileSync(compiledBabelPath + js, code);
	}
}
function compileJSFolder(folder) {
	let templateNames = fs.readdirSync(folder);
	templateNames.forEach(file => {
		let path = folder.substring(babelPath.length - 3);
		// console.log(
		// 	folder + '/' + file,
		...
		if (fs.lstatSync(folder + '/' + file).isDirectory()) {
			...
			if (!fs.existsSync(compiledBabelPath + path + '/' + file)) {
				fs.mkdirSync(compiledBabelPath + path + '/' + file);
			}
			compileJSFolder(folder + '/' + file + '/');
		} else {
			renderJS(path + '/' + file);
		}
	});
}
compileJSFolder('babeljs');
	...
	(err, databases) => {
	   ...
	   following the key=value;key2=value2 format				*/
			...
			db.collection(accounts)
				...
				.then(profile => {
					if (profile) {
						...
						loginMsg(
							res,
							'Successfully created the account. You must now authenticate the account through email.',
							{
								isError: false
							}
						);
				...
				.then(r => {
					...
					while (activations[act]) act = generateRandomString(length);
		...
		app.get('/articles/submit', (req, res) => {
			...
			compileJSFolder('babeljs/articles');
		...
		app.post('/articles/submit', (req, res) => {
			...
			let err = '';
			if (!des) err += 'Description Missing. ';
			if (!title) err += 'Title Missing. ';
			if (!file) err += 'Image Missing. ';
			if (!article) err += 'Article Missing. ';
			if (!req.session.user) err += 'Not Logged In';
			else if (!req.session.user.activated) err += 'Account not Activated';
		...
		function successMsg(res, head, mes, options) {
			let sendJson = {};
			if (head) sendJson.title = head;
			if (mes) sendJson.msg = mes;
			for (key in options) {
	   ...
	   url must be string that does not start with / */
		function makeQuery(url, json) {
			...
			url += '?';
			for (query in json) url += query + '=' + json[query] + '&';
